...
public class RetourPan extends JPanel implements ActionListener {

    private JTextField txt_client = new JTextField();

    private JLabel lbl_client = new JLabel("Client : ");
    private JLabel lbl_client_value = new JLabel(""); // Affiche le nom du client
    ...
    private JButton btn_confirm = new JButton("Confirmer le retour");
    ...
    private JTable tbl_livres_retour = new JTable();
    ...
    public RetourPan() {
        ...
        this.txt_scan.setActionCommand("Ajout retour");
        this.add(txt_scan);
        ...
        tbl_livres_retour.setModel(model_livres);

        JScrollPane jsp = new JScrollPane(tbl_livres_retour);
        ...
        switch (e.getActionCommand()) {
            ...
            case "Ajout retour":
                ...
                if (!this.txt_client.getText().isBlank()) {
                    ...
                    if (verifCodeBarreDejaPresent(tbl_livres_retour, codeBarre)) {
                        JOptionPane.showMessageDialog(null, "Ce livre est déjà dans la liste des retours.", "Duplication", JOptionPane.WARNING_MESSAGE);
                    } else if (verifLivreDejaRetourne(codeBarre)) {
                        // Si le livre est déjà présent, proposer de forcer le retour
                        int choixUser = JOptionPane.showConfirmDialog(null, "Le livre a déjà été retourné. Voulez-vous forcer le retour ?", "Retour déjà effectué", JOptionPane.YES_NO_OPTION);
                        if (choixUser == JOptionPane.YES_OPTION) {
                            modifEtatExemplaire(codeBarre, "Emprunté");
                            scanCodeBarreExemplaires(tbl_livres_retour, codeBarre);
                        }
                    } else {
    ...
    private void resultatRechercheClient(RetourPan retourPan) {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                ...
                if (row >= 0) {
                    if (e.getClickCount() == 2) { // Détecte un double-clic
                        ...
                        retourPan.txt_client.setText("");
                        retourPan.lbl_client_id.setText(String.valueOf(model_rech.getValueAt(row, 0)));
                        retourPan.lbl_client_value.setText(model_rech.getValueAt(row, 1) + " " + model_rech.getValueAt(row, 2));
                        dialog_rech.dispose();
                    }
                }
            }
    ...
    private void SuppExemplaireSelection() {
        ...
        int selectedRow = tbl_livres_retour.getSelectedRow();

        if (selectedRow != -1) { // Vérifie qu'une ligne est bien sélectionnée
            DefaultTableModel model = (DefaultTableModel) tbl_livres_retour.getModel();
    ...
    private boolean verifCodeBarreDejaPresent(JTable tbl_livres_retour, String codeBarre) {
    ...
    private boolean verifLivreDejaRetourne(String codeBarre) {
        ...
        return exemplaire.getEtat().equals("Disponible");
    }

    private void modifEtatExemplaire(String codeBarre, String emprunte) {

        DAOExemplaires daoExemplaire = new DAOExemplaires();

        daoExemplaire.updateEtat(codeBarre, emprunte);
    ...
    private void confirmationRetour() {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            emprunt.setDate_retour_reel(LocalDate.now());

            try {
                daoEmprunt.update(emprunt);
                daoExemplaires.update(exemplaire);
            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(this, "Erreur lors de l'enregistrement du retour du/des livre(s) : " + codeBarres, "Erreur", JOptionPane.ERROR_MESSAGE);
            }
        }
        JOptionPane.showMessageDialog(this, "Retour(s) enregistré(s) avec succès !", "Confirmation", JOptionPane.INFORMATION_MESSAGE);
    }
}
